...
PATTERN_GREETING_ONLY = re.compile(
    ...
    ^\s*
    (
        b+o+n+j+o+u+r+       # bonjour et variantes etirées
        | h+e+l+l+o+          # hello, helllo...
        | h+i+                # hi, hii, hiii...
        | h+e+y+              # hey, heyy...
        | b+o+n+s+o+i+r+     # bonsoir
        | s+a+l+u+t+          # salut
        | good\s+morning
        | good\s+evening
        | oui | non | yes | no
...
PATTERN_TICKET_ID = re.compile(r"\b(?:CS|INC)\d{8}\b", re.I)
...
def get_transcript(conversations: pd.DataFrame) -> pd.DataFrame:
    ...
    # Exclure les conversations CSAT (assignee contient "csat" ou queue == "CSAT Survey")
    convs = conversations[
        ~conversations["assignee"].str.contains("csat", case=False, na=False)
        & ~conversations["queue"].str.contains("CSAT Survey", case=False, na=False)
    ]

    transcript = (
        convs
        .assign(messages=convs["transcript"].str.findall(pattern_msg))
        .explode("messages")
...
def get_questions(transcript: pd.DataFrame) -> pd.DataFrame:
    ...
    df["user_hotel_code_only"] = (
        (df["speaker_type"] == "user")
        & df["message_clean"].str.fullmatch(r"[Hh][A-Za-z0-9]{4}", na=False)
        & df["message_clean"].str.contains(r"\d", na=False)
        & ~df["message_clean"].str.upper().eq("HXXXX")
    )
    ...
    # Messages avant le premier user → rattachés à Q1 (hors greetings et hotel codes)
    df.loc[
        (df["has_user_message"])
        & (df["question_rank"] == 0)
        & ~df["user_greeting_only"]
...
# H + 4 caractères alphanumériques avec au moins un chiffre (exclut "Hello", "Hiii", etc.)
HOTEL_CODE_PATTERN = re.compile(r'\b[Hh](?=[A-Za-z0-9]{4}\b)(?=[A-Za-z]*\d)[A-Za-z0-9]{4}\b')
...
def get_hotel_code(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cols = ["id", "hotelCode", "transcript"]
    result = (
        df[cols].groupby("id")
        .apply(resolve_hotel_code)
        .reset_index()
    )
    result.columns = ["id", "hotel_code"]
    return result

def get_tickets_from_conversations(conversations: pd.DataFrame) -> pd.DataFrame:
    """
    Extrait les numéros de tickets mentionnés dans les transcripts par des agents humains.
    Remplace le fichier 'tickets with CID' : retourne un DataFrame avec
    les colonnes ['Numéro'] correspondant aux tickets créés via Butler.
    
    Formats supportés : CS########, INC########
    """
    transcript = get_transcript(conversations)
    questions = get_questions(transcript)
    agent_msgs = questions[questions["speaker_type"] == "human_agent"].copy()
 
    agent_msgs["ticket_id"] = agent_msgs["message"].str.extract(
        r"\b((?:CS|INC)\d{8})\b", flags=re.I
    )[0].str.upper()
 
    tickets = (
        agent_msgs[agent_msgs["ticket_id"].notna()][["ticket_id"]]
        .drop_duplicates()
        .rename(columns={"ticket_id": "Numéro"})
