...
class App(tk.Tk):
    def __init__(self):
        ...
        self.dfs["hotels"] = None

        # Choix pour la liste hôtels : "default" (fichier embarqué) ou "custom" (upload)
        self.hotel_choice = tk.StringVar(value="default")
        self.hotel_custom_path = tk.StringVar()

        self._build_ui()

        # Charge la liste hôtels par défaut au démarrage
        try:
            self.dfs["hotels"] = read_file(HOTEL_LIST_PATH)
        except Exception as e:
            messagebox.showwarning(
                "Liste hôtels introuvable",
                f"Impossible de charger {HOTEL_LIST_PATH} :\n{e}\n\n"
                f"Sélectionne 'Uploader une autre liste' pour en fournir une."
            )
            self.dfs["hotels"] = None
    ...
    def _build_ui(self):
        ...
        # Choix liste hôtels
        tk.Label(self, text="Liste hôtels :", font=FONT_LABEL, bg=BG).grid(
            row=row, column=0, columnspan=3, sticky="w")
        row += 1

        tk.Radiobutton(
            self, text="Utiliser la liste intégrée au programme",
            variable=self.hotel_choice, value="default", bg=BG,
            font=FONT_LABEL, command=self._on_hotel_choice_change
        ).grid(row=row, column=0, columnspan=3, sticky="w")
        row += 1

        tk.Radiobutton(
            self, text="Uploader une autre liste",
            variable=self.hotel_choice, value="custom", bg=BG,
            font=FONT_LABEL, command=self._on_hotel_choice_change
        ).grid(row=row, column=0, columnspan=3, sticky="w")
        row += 1

        self.hotel_entry = tk.Entry(self, textvariable=self.hotel_custom_path,
                                     width=52, state="disabled")
        self.hotel_entry.grid(row=row, column=0, columnspan=2, sticky="ew")

        self.btn_hotel_browse = tk.Button(
            self, text="Parcourir…", font=FONT_BTN, state="disabled",
            command=self._browse_hotel_list)
        self.btn_hotel_browse.grid(row=row, column=2, padx=(8, 0))
        row += 1
    ...
    # ------------------------------------------------------------------
    def _on_hotel_choice_change(self):
        """Active/désactive le champ + bouton selon le choix radio."""
        if self.hotel_choice.get() == "custom":
            self.hotel_entry.config(state="normal")
            self.btn_hotel_browse.config(state="normal")
            # Si pas encore de fichier choisi, le bouton run reste bloqué tant
            # qu'aucun fichier custom n'est sélectionné.
            if not self.hotel_custom_path.get():
                self.dfs["hotels"] = None
        else:
            self.hotel_entry.config(state="disabled")
            self.btn_hotel_browse.config(state="disabled")
            try:
                self.dfs["hotels"] = read_file(HOTEL_LIST_PATH)
            except Exception as e:
                messagebox.showerror("Erreur liste hôtels", str(e))
                self.dfs["hotels"] = None
        self._try_unlock()

    # ------------------------------------------------------------------
    def _browse_hotel_list(self):
        path = filedialog.askopenfilename(
            title="Sélectionner la liste hôtels",
            filetypes=[("Excel / CSV", "*.xlsx *.csv"), ("Tous", "*.*")]
        )
        if not path:
            return
        self.hotel_custom_path.set(path)
        try:
            self.dfs["hotels"] = read_file(path)
        except Exception as e:
            messagebox.showerror("Erreur de lecture", str(e))
            self.dfs["hotels"] = None
            return
    ...
    def _try_unlock(self):
        ...
        # Active le bouton dès que conv + tickets + hôtels sont chargés
        required_ok = all(self.dfs[k] is not None for k, _ in FILE_SLOTS)
        hotels_ok = self.dfs["hotels"] is not None

        if required_ok and hotels_ok:
